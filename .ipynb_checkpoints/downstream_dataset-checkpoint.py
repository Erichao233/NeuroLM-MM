...
class HMC_ECG_Loader(Dataset):
    def __init__(self, root, files, sampling_rate=200, eeg_max_len=-1, text_max_len=-1, is_instruct=False, is_val=False):
        ...
        if is_instruct:
            ...
            # 50257 for [SEP]
            self.text = {
                0: torch.IntTensor([50257] + encode('Question: Which sleep type does this ECG segment belong to? Options: (A) Wake. (B) NREM-1. (C) NREM-2. (D) NREM-3. (E) REM. Answer: (A) <|endoftext|>')),
                1: torch.IntTensor([50257] + encode('Question: Which sleep type does this ECG segment belong to? Options: (A) Wake. (B) NREM-1. (C) NREM-2. (D) NREM-3. (E) REM. Answer: (B) <|endoftext|>')),
                2: torch.IntTensor([50257] + encode('Question: Which sleep type does this ECG segment belong to? Options: (A) Wake. (B) NREM-1. (C) NREM-2. (D) NREM-3. (E) REM. Answer: (C) <|endoftext|>')),
                3: torch.IntTensor([50257] + encode('Question: Which sleep type does this ECG segment belong to? Options: (A) Wake. (B) NREM-1. (C) NREM-2. (D) NREM-3. (E) REM. Answer: (D) <|endoftext|>')),
                4: torch.IntTensor([50257] + encode('Question: Which sleep type does this ECG segment belong to? Options: (A) Wake. (B) NREM-1. (C) NREM-2. (D) NREM-3. (E) REM. Answer: (E) <|endoftext|>')),
            }
            self.prompt = torch.IntTensor([50257] + encode('Question: Which sleep type does this ECG segment belong to? Options: (A) Wake. (B) NREM-1. (C) NREM-2. (D) NREM-3. (E) REM. Answer: ('))
...
class HMC_EEG_ECG_Loader(Dataset):
    """
    Pair EEG samples from HMC with ECG samples from HMC_ECG by (record_id, epoch_idx),
    concatenate channels, and return a unified sample for instruction tuning.
    """
    def __init__(self, eeg_root, eeg_files, ecg_root, ecg_files, sampling_rate=200, eeg_max_len=-1, text_max_len=-1, is_instruct=False, is_val=False):
        ...
        # build index by key: (record_id, epoch_idx)
        import re
        eeg_map = {}
        for f in eeg_files:
            m = re.match(r"^(SN\d+)-(\d+)\.pkl$", f)
            if not m:
                continue
        ...
        ecg_map = {}
        for f in ecg_files:
            m = re.match(r"^(SN\d+)-ecg-(\d+)\.pkl$", f)
            if not m:
                continue
            key = (m.group(1), int(m.group(2)))
            ecg_map[key] = f

        keys = sorted(list(set(eeg_map.keys()) & set(ecg_map.keys())))
        self.pairs = [(eeg_map[k], ecg_map[k]) for k in keys]

        if is_instruct:
            enc = tiktoken.get_encoding("gpt2")
            encode = lambda s: enc.encode(s, allowed_special={"<|endoftext|>"})
            # 50257 for [SEP]
            note = 'Note: modality embeddings used are 0=EEG and 1=ECG.'
            q = 'Question: Which sleep type does this EEG+ECG segment belong to? '
            opts = 'Options: (A) Wake. (B) NREM-1. (C) NREM-2. (D) NREM-3. (E) REM.'
            self.text = {
                0: torch.IntTensor([50257] + encode(f'{q}{opts} {note} Answer: (A) <|endoftext|>')),
                1: torch.IntTensor([50257] + encode(f'{q}{opts} {note} Answer: (B) <|endoftext|>')),
                2: torch.IntTensor([50257] + encode(f'{q}{opts} {note} Answer: (C) <|endoftext|>')),
                3: torch.IntTensor([50257] + encode(f'{q}{opts} {note} Answer: (D) <|endoftext|>')),
                4: torch.IntTensor([50257] + encode(f'{q}{opts} {note} Answer: (E) <|endoftext|>')),
            }
            self.prompt = torch.IntTensor([50257] + encode(f'{q}{opts} {note} Answer: ('))

    def __len__(self):
        return len(self.pairs)

    def __getitem__(self, index):
        eeg_file, ecg_file = self.pairs[index]
        eeg_sample = pickle.load(open(os.path.join(self.eeg_root, eeg_file), "rb"))
        ecg_sample = pickle.load(open(os.path.join(self.ecg_root, ecg_file), "rb"))

        X_eeg = eeg_sample["X"]
        X_ecg = ecg_sample["X"]
        Y_eeg = int(eeg_sample["y"]) if isinstance(eeg_sample["y"], (int, np.integer)) else int(eeg_sample["y"][0])
        Y_ecg = int(ecg_sample["y"]) if isinstance(ecg_sample["y"], (int, np.integer)) else int(ecg_sample["y"][0])
        assert Y_eeg == Y_ecg, "EEG and ECG labels mismatch for paired sample"
        Y = Y_eeg

        # concatenate channels along C
        X = np.concatenate([X_eeg, X_ecg], axis=0)
        ch_names = list(eeg_sample["ch_names"]) + list(ecg_sample["ch_names"])  # e.g., ['F4','C4','O2','C3'] + ['ECG']
        ...
        valid_eeg_len = data.size(0)
        if self.eeg_max_len > data.size(0):
            X_eeg_tokens = torch.zeros((self.eeg_max_len, 200))
            X_eeg_tokens[:data.size(0)] = data
            eeg_mask = torch.ones(self.eeg_max_len)
            eeg_mask[valid_eeg_len:] = 0
        ...
        num_tokens = X_eeg_tokens.size(0) + text.size(0)
        gpt_mask = torch.tril(torch.ones(num_tokens, num_tokens)).view(1, num_tokens, num_tokens)
        num_chans = len(ch_names)
        for i in range(time):
            gpt_mask[:, i * num_chans:(i + 1) * num_chans,  i * num_chans:(i + 1) * num_chans] = 1
        gpt_mask[:, :, valid_eeg_len:X_eeg_tokens.size(0)] = 0
        
        if self.is_val:
            return X_eeg_tokens, text, Y, input_chans, input_time, eeg_mask.bool(), gpt_mask.bool()
        
        Y_text = torch.full_like(text, fill_value=-1)
        prompt_len = self.prompt.size(0) - 1
        valid_text_len = (text != 50256).sum().item() if self.text_max_len > 0 else text.size(0)
        Y_text[prompt_len - 1:valid_text_len - 1] = text[prompt_len:valid_text_len]
        return X_eeg_tokens, text, Y_text, input_chans, input_time, eeg_mask.bool(), gpt_mask.bool()
...
class HMC_EEG_ECG_PretrainLoader(Dataset):
    """
    For multimodal MC-AR pretraining on HMC: pairs EEG with ECG by (record_id, epoch_idx),
    concatenates channels and returns inputs for autoregressive training (no text).
    Outputs: X, input_chans(idx), input_time, input_mask(bool ones), gpt_mask(bool), num_chans(int), num_tokens(int)
    """
    def __init__(self, eeg_root, eeg_files, ecg_root, ecg_files, sampling_rate=200, block_size=1024):
        self.eeg_root = eeg_root
        self.ecg_root = ecg_root
        ...
        for f in eeg_files:
            m = re.match(r"^(SN\d+)-(\d+)\.pkl$", f)
            if m:
                eeg_map[(m.group(1), int(m.group(2)))] = f
        ecg_map = {}
        for f in ecg_files:
            m = re.match(r"^(SN\d+)-ecg-(\d+)\.pkl$", f)
            if m:
                ecg_map[(m.group(1), int(m.group(2)))] = f
        keys = sorted(list(set(eeg_map.keys()) & set(ecg_map.keys())))
        self.pairs = [(eeg_map[k], ecg_map[k]) for k in keys]
        self.ecg_name = 'ECG'
    ...
    def __getitem__(self, index):
        ...
        X_ecg = ecg_sample["X"]
        X = np.concatenate([X_eeg, X_ecg], axis=0)
        ch_names = [*eeg_sample["ch_names"], *ecg_sample["ch_names"]]
        ...
        # pad to at most block_size if needed (truncate if longer)
        tokens = data.size(0)
        block_size = min(self.block_size, tokens)
        X_block = data[:block_size]
        input_mask = torch.ones(block_size)
        input_chans = (ch_names * time)[:block_size]
        input_time = torch.IntTensor(input_time[:block_size])
        input_chans = torch.IntTensor(get_chans(input_chans))

        # gpt causal mask with within-time-step fully visible approximation by channel grouping
        # Here we approximate by allowing blocks of size num_chans as fully visible.
        num_chans = len(ch_names)
        gpt_mask = torch.tril(torch.ones(block_size, block_size)).view(1, block_size, block_size)
        for i in range(min(time, block_size // num_chans)):
            s = i * num_chans
            e = min((i + 1) * num_chans, block_size)
            gpt_mask[:, s:e, s:e] = 1

        return X_block, input_chans, input_time, input_mask.bool(), gpt_mask.bool(), num_chans, block_size


class HMC_Peripheral_PretrainLoader(Dataset):
    """
    Build peripheral (EOG+ECG+EMG) context by pairing available modalities per (SN, epoch).
    Missing modalities are skipped; channels are concatenated. Used as Stage-1 fusion input for MC-AR.
    """
    def __init__(self, eog_root=None, eog_files=None, ecg_root=None, ecg_files=None, emg_root=None, emg_files=None, block_size=1024):
        import re
        self.block_size = block_size
        maps = []
        if eog_root and eog_files:
            m = {}
            for f in eog_files:
                mm = re.match(r"^(SN\d+)-eog-(\d+)\.pkl$", f)
                if mm: m[(mm.group(1), int(mm.group(2)))] = ('eog', f)
            maps.append((eog_root, m))
        if ecg_root and ecg_files:
            m = {}
            for f in ecg_files:
                mm = re.match(r"^(SN\d+)-ecg-(\d+)\.pkl$", f)
                if mm: m[(mm.group(1), int(mm.group(2)))] = ('ecg', f)
            maps.append((ecg_root, m))
        if emg_root and emg_files:
            m = {}
            for f in emg_files:
                mm = re.match(r"^(SN\d+)-emg-(\d+)\.pkl$", f)
                if mm: m[(mm.group(1), int(mm.group(2)))] = ('emg', f)
            maps.append((emg_root, m))
        # intersect keys
        if not maps:
            self.pairs = []
            return
        keys = set(maps[0][1].keys())
        for _,mp in maps[1:]: keys &= set(mp.keys())
    ...
    def __getitem__(self, index):
        ...
        xs = []
        chs = []
        for root, mp in self.maps:
            mod, f = mp[key]
            s = pickle.load(open(os.path.join(root, f), 'rb'))
            xs.append(s['X'])
            chs.extend(s['ch_names'])
        ...
        X_block = data[:block_size]
        input_mask = torch.ones(block_size)
        input_chans = (chs * time)[:block_size]
        input_time = torch.IntTensor(input_time[:block_size])
        input_chans = torch.IntTensor(get_chans(input_chans))
        num_chans = len(chs)
        gpt_mask = torch.tril(torch.ones(block_size, block_size)).view(1, block_size, block_size)
...
class HMC_EEG_Peripheral_PretrainLoader(Dataset):
    """
    Pair EEG (HMC) with peripheral (EOG+ECG+EMG) by (SN, epoch). Concatenate channels as [EEG | PERIPH].
    Output format matches MC-AR pretrain: X_block, input_chans(idx), input_time, input_mask, gpt_mask, num_chans_total, block_tokens
    """
    def __init__(self, eeg_root, eeg_files, eog_root=None, eog_files=None, ecg_root=None, ecg_files=None, emg_root=None, emg_files=None, block_size=1024):
        import re
        self.block_size = block_size
        # maps
        eeg_map = {}
        ...
        if eog_root and eog_files:
            ...
            for f in eog_files:
                mm = re.match(r"^(SN\d+)-eog-(\d+)\.pkl$", f)
                if mm: m[(mm.group(1), int(mm.group(2)))] = ('eog', f)
            periph_maps.append((eog_root, m))
        if ecg_root and ecg_files:
            m = {}
            for f in ecg_files:
                mm = re.match(r"^(SN\d+)-ecg-(\d+)\.pkl$", f)
                if mm: m[(mm.group(1), int(mm.group(2)))] = ('ecg', f)
            periph_maps.append((ecg_root, m))
        if emg_root and emg_files:
            m = {}
            for f in emg_files:
                mm = re.match(r"^(SN\d+)-emg-(\d+)\.pkl$", f)
                if mm: m[(mm.group(1), int(mm.group(2)))] = ('emg', f)
            periph_maps.append((emg_root, m))
        # intersect keys across all present maps
        if not periph_maps:
            self.pairs = []
            return
        keys = set(eeg_map.keys())
        for _,mp in periph_maps:
            keys &= set(mp.keys())
        self.keys = sorted(list(keys))
        self.eeg_root = eeg_root
        self.eeg_map = eeg_map
        self.periph_maps = periph_maps
    ...
    def __getitem__(self, index):
        ...
        key = self.keys[index]
        # EEG
        eeg_sample = pickle.load(open(os.path.join(self.eeg_root, self.eeg_map[key]), 'rb'))
        Xs = [eeg_sample['X']]
        chs = list(eeg_sample['ch_names'])
        eeg_chans = len(chs)
        # PERIPH
        for root, mp in self.periph_maps:
            mod, f = mp[key]
            s = pickle.load(open(os.path.join(root, f), 'rb'))
            Xs.append(s['X'])
            chs.extend(s['ch_names'])
        ...
        for i in range(min(time, block_size // num_chans)):
            ...
            e = min((i + 1) * num_chans, block_size)
            gpt_mask[:, s:e, s:e] = 1
        return X_block, input_chans, input_time, input_mask.bool(), gpt_mask.bool(), num_chans, block_size, eeg_chans
